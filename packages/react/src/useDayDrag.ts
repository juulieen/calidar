/**
 * Pointer-driven drag/create/resize snapped to *whole days*.
 *
...
export interface DayDragHandlers {
  active: ActiveDayDrag | null;
  /** Begin a create gesture anchored on column `col`. */
  startCreate: (e: React.PointerEvent, col: number) => void;
...
export function useDayDrag(opts: UseDayDragOptions): DayDragHandlers {
  const [active, setActiveState] = useState<ActiveDayDrag | null>(null);
  // Mirror `active` in a ref so the once-registered `pointerup` listener reads
  // the latest preview instead of a stale closure (see useGridDrag).
  const activeRef = useRef<ActiveDayDrag | null>(null);
  const setActive = useCallback((next: ActiveDayDrag | null) => {
    activeRef.current = next;
    setActiveState(next);
  }, []);
  const stateRef = useRef<DayDragState | null>(null);
  const startXY = useRef<{ x: number; y: number }>({ x: 0, y: 0 });

  const compute = useCallback(
    (s: DayDragState, hoverCol: number): ActiveDayDrag | null => {
