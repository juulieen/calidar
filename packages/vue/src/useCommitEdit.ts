/**
 * Shared commit logic for a drag/resize that lands on an existing event (Vue
 * port).
 *
 * - Non-recurring instance → patch the store directly + fire `onEventUpdate`.
 * - Recurring instance → defer: surface a pending edit (reactive ref) so the
 *   view can render the scope popover, then apply `editRecurringEvent`'s pure
 *   result once the user picks a scope.
 *
...
export interface CommitEditApi {
  /** The edit waiting for a scope choice, or null (reactive). */
  pending: Ref<PendingRecurringEdit | null>;
  /**
...
export function useCommitEdit(): CommitEditApi {
  const { store, snapshot, onEventUpdate, onRecurringEdit } =
    useCalendarContext();
  const pending = shallowRef<PendingRecurringEdit | null>(null);
  ...
  const commit = (
    ...
    if (instance.recurring) {
      pending.value = { instance, occurrenceStart: instance.start, patch };
      return;
    }
  ...
  const resolve = (scope: RecurrenceEditScope): void => {
    ...
    if (!current) return;
    const { instance, occurrenceStart, patch } = current;
    pending.value = null;

    const handled = onRecurringEdit?.({
      instance,
      occurrenceStart,
      patch,
      scope,
    ...
    if (!handled) {
      const mutation = editRecurringEvent({
        ...
        scope,
        patch,
        timeZone: snapshot.value.state.timeZone,
      });
